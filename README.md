...
The frontend application is located in the `Project_api_web` folder and provides four separate portals:

### 1. 👩‍💼 Admin Portal
- Used for **student data entry**
- Adds student details and photo into the system

### 2. 🏨 Hostel (Warden) Portal
- Used when a student **leaves the hostel** to go to the library
- Records the **hostel out time** and generates a QR code

### 3. 📖 Library Portal
- Used for **library exit logging**
- Records the **library out time**, updates `LibraryLogs` table, and links with the QR entry

### 4. 🧑‍🎓 Student Portal
- Students must **register with the email** stored in the database
- After login, students can **view their QR codes** showing out time (hostel or library)
